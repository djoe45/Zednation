		...
		if (count(_this getVariable ["actions",[]]) == 0) then { 
			...
			if (typeOf _this == "Land_Camping_Light_F") then {
				...
				if ((_this getVariable ["isBattery", false]) or (_this getVariable ["isGenerator", false])) then {
					...
					_action_2 = _this addAction [format[localize "STR_DAYZ_CODE_1",getText (configFile >> "CfgVehicles" >> "Land_Camping_Light_off_F" >> "displayName")], {[_this select 0,_this select 1] call (_this select 3)}, fnc_code_take, 1, true, true];
				} else {	
