...
if((count _Structures > 0) and !_inVehicle and _canDo) then {
	if !(s_player_packtent select 1 == typeOf(_Structures select 0)) then {
		if (s_player_packtent select 0 < 0) then {
			s_player_packtent = [player addAction [format[localize "str_actions_Pack",getText(configFile >> "CfgVehicles" >> typeOf (_Structures select 0) >> "displayName")], "\z\addons\dayz_code\actions\tent_pack.sqf",_Structures select 0, 0, false, true],typeOf(_Structures select 0)];
		} else {
...
if((unitBackpack player isKindOf "B_VehicleBag_base") and !_inVehicle and _canDo) then {
	if !(s_player_assembleVehicle select 1 == typeOf(unitBackpack player)) then {	
		if (s_player_assembleVehicle  select 0 < 0) then {
			s_player_assembleVehicle = [player addAction [format[localize "str_actions_Assemble",getText(configFile >> "CfgVehicles" >> typeOf(unitBackpack player) >> "displayName")], "\z\addons\dayz_code\actions\assemble_vehicle.sqf",typeOf(unitBackpack player), 0, false, true],typeOf(unitBackpack player)];
		} else {
...
if((count _vehicles > 0) and !_inVehicle and _canDo) then {
	if !(s_player_disassembleVehicle select 1 == typeOf(_vehicles select 0)) then {	
		if (s_player_disassembleVehicle select 0 < 0) then {
			s_player_disassembleVehicle = [player addAction [format[localize "str_actions_Disassemble",getText(configFile >> "CfgVehicles" >> typeOf (_vehicles select 0) >> "displayName")], "\z\addons\dayz_code\actions\disassemble_vehicle.sqf",_vehicles select 0, 0, false, true],typeOf (_vehicles select 0)];
		} else {
...
if (!isNull cursorTarget and !_inVehicle and (player distance cursorTarget < 4)) then {	//Has some kind of target
	...
	if (_hasFuelE) then {
		_isFuel = (cursorTarget isKindOf "Land_Tank_rust_F") or (cursorTarget isKindOf "Land_fs_feed_F");
	};
	...
	*/
	...
	if(cursorTarget isKindOf "TentStorage_base" and _canDo) then {
		if ((s_player_packtent < 0) and (player distance cursorTarget < 3)) then {
			...
			s_player_packtent = player addAction [format[localize "str_actions_Pack",getText(configFile >> "CfgVehicles" >> typeOf cursorTarget >> "displayName")], "\z\addons\dayz_code\actions\tent_pack.sqf",cursorTarget, 0, false, true, "",""];
		};
	} else {
