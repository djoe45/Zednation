...
_players = _position nearEntities ["AllPlayersClass",_radius+200];
//dayz_maxGlobalZombies = 40;
dayz_maxGlobalZombies = dayz_maxLocalZombies;
{
...
*/	
_nearBy = nearestObjects [_position, ["building","Maniken_Base","WeaponsBox_SRV"], _radius];
...

{
	...
	if ((_dis < 120) and (_dis > 30) and _canLoot and !_inVehicle) then {
		...
		if ((_age > 10) and (!_cleared)) then {
			_nearByObj = nearestObjects [(getPosATL _x), ["GroundWeaponHolder","ReammoBox"],((sizeOf _type)+5)];
		...
		if (dayz_maxCurrentZeds < dayz_maxZeds) then {
			if (dayz_CurrentZombies < dayz_maxGlobalZombies) then {
				if (dayz_spawnZombies < dayz_maxLocalZombies) then {
						...
						//if (_age > 3) then {
						if (_age > dayz_zedsRespawnDelay) then {
