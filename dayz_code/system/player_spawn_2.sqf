...
dayz_myLoad = loadAbs player;

(findDisplay 46) displaySetEventHandler ["KeyDown", "_this call player_keyhandler"];
...
while {true} do {
	...
	*/
	...
	_hunger = +(((r_player_bloodTotal - r_player_blood) / r_player_bloodTotal) * 5) + (_speed * 3) + dayz_myLoad;
	if (time - dayz_panicCooldown < 120) then {
		_hunger = _hunger * 2;
	};
	dayz_hunger = dayz_hunger + (_hunger / 360);
