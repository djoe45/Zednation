...
#define VEH_FORCE_SPAWN (({_x  isKindOf "AllVehicles"} count dayz_serverObjectMonitor < 20) && _vehicleForceSpawn)
...
while {true} do {
	...
	if (_spawnRoll <= _spawnChance || VEH_FORCE_SPAWN) then {
		if ((count (allMissionObjects (_vehicleModel select 0))) <  (_vehicleModel select 1)) then {
			...
			//Send request
			_key = format["CHILD:308:%1:%2:%3:%4:%5:%6:%7:%8:%9:",dayZ_instance, (typeOf _vehicle), 0 , _charID, _worldspace, [], _array, _Fuel,_uid];
